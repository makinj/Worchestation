class Guest
  def initialize(globalvagrant, config, provider_callbacks=[], vm_callbacks=[], callbacks=[])
    ...
    if @config.has_key?("workspaces_path") 
      setup_workspace_dir(@config['workspaces_path'])
    end
  ...
  def setup_workspace_dir(workspaces_path)
    workspace_path="#{workspaces_path}/#{@name}/sf"
